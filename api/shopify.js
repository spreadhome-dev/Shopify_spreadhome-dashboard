...
async function handleDashboard(days = 30) {
  ...
  orders.forEach(o => {
    ...
    revByDay[day] = (revByDay[day] || 0) + parseFloat(o.total_price || 0);
  });
  ...
  orders.forEach(o => {
    ...
    bySource[src].orders++;
    bySource[src].revenue += parseFloat(o.total_price || 0);
  });
  ...
  orders.forEach(o => {
    (o.line_items || []).forEach(li => {
      ...
      prodMap[li.title].revenue += parseFloat(li.price || 0) * li.quantity;
    });
  });
  ...
  const payload = {
    ...
    products: products.slice(0, 100).map(p => ({
      ...
      variants: (p.variants || []).map(v => ({
        sku: v.sku, price: parseFloat(v.price || 0), inventory: v.inventory_quantity,
      })),
    })),
  };
